...
class CorpusReader:
    ...
    def __init__(self, dataset_file=None, stopwords_file=None,
        ...
        if dataset_file is not None:
            ...
            if(len(dataset[0])==4):
                self.train_set, self.valid_set, self.test_set = self.divide_data_set_topic_style(dataset, train_valid_test_rate)
    ...
    def divide_data_set(self, doc_list, train_valid_test_rate):
        ...
        valid_set=doc_list[n_train:n_train_valid_test]
        test_set = doc_list[n_train:n_train_valid_test]
    ...
    def divide_data_set_topic_style(self, doc_list, train_valid_test_rate):
        n_train = int(math.floor(train_valid_test_rate[0] * len(doc_list)))
        n_train_valid = int(math.floor((train_valid_test_rate[0] +
                                        train_valid_test_rate[1]) * len(doc_list)))
        n_train_valid_test = int(math.floor((train_valid_test_rate[0] +
        ...
        valid_set = doc_list[n_train:n_train_valid_test]
        test_set = doc_list[n_train_valid:n_train_valid_test]
        train_set_q, train_set_a,train_set_s,train_set_t = zip(*train_set)
        valid_set_q, valid_set_a,valid_set_s,valid_set_t = zip(*valid_set)
        test_set_q, test_set_a,test_set_s,test_set_t = zip(*test_set)

        return (train_set_q, train_set_a,train_set_s,train_set_t), (valid_set_q, valid_set_a,valid_set_s,valid_set_t), (test_set_q, test_set_a,test_set_s,test_set_t)
