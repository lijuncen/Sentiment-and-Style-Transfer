...
def beam_search_t(sentence, cr, deploy_model,answer_dict, minLen=9, maxLen = 20, search_scope = 200,
    ...
    def step(last_sentence, score, pred_words_prob_array_old,pred_word_array_old,beam_small_value,beam_full_flag):
        ...
        cands = list()
        #for pred_word, pred_word_prob in sorted_index:
        #for i in range(search_scope):
        for i in range(min(len(pred_words_prob_array),10)):
    ...
    topic_label=[[string.atoi(sentence_topic[2])]]
    #sentence=sentence_topic[0]
    (question, question_mask) = cr.transform_input_data(sentence_topic[0])
    (style,style_mask) =cr.transform_input_data(sentence_topic[1])
    base_length = question.shape[0]
    (tanswer, tanswer_mask) = (question[-1:,:], question_mask[-1:,:])
    pred_word, pred_words_prob = deploy_model(question[:-1,:], question_mask[:-1,:], tanswer, tanswer_mask,style[:-1,:], style_mask[:-1,:],topic_label)
    sentence = load_sentence('', cr.word2index, special_flag=cr.special_flag)
    sentence=[sentence[1]]
    ...
    for iter in xrange(maxLen):
        ...
        ext_question = numpy.concatenate([question]*tanswer.shape[1], axis=1)
        ext_question_mask = numpy.concatenate([question_mask]*tanswer.shape[1], axis=1)
        ext_style = numpy.concatenate([style]*tanswer.shape[1], axis=1)
        ext_style_mask = numpy.concatenate([style_mask]*tanswer.shape[1], axis=1)
        ext_topic = numpy.concatenate([topic_label]*tanswer.shape[1], axis=1).astype('float32')
        pred_word, pred_words_prob = deploy_model(ext_question[:-1,:], ext_question_mask[:-1,],
                                                  tanswer, tanswer_mask,ext_style[:-1,:],ext_style_mask[:-1,:],ext_topic)
