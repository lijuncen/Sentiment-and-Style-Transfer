...
class CorpusReaderDialog(CorpusReader):
    ...
    def __init__(self, dataset_file=None, min_dialog_sentence_num=2, \
        ...
        self.pr_set = self.load_data(measure_base_dir + '/PR_test', charset)
        self.pr_set = zip(*self.pr_set)
        
    def get_pr_set(self, scope=None, shuffle=False, merge=False):
        return self.get_model_input(None, self.pr_set[0], self.pr_set[1], \
    ...
    def get_model_input(self, scope, question, answer, shuffle=False, merge=False):
        ...
        if not merge:
            ...
            return (question_data, question_mask), (answer_data, answer_mask), batch_question, batch_answer  
        else:
            batch = [question + answer for (question, answer) in zip(batch_question, batch_answer)]
    ...
    def load_data(self, dataset_file, charset='utf-8'):
        ...
        def deal_one_sentence(lines):
            ...
            for line in lines:
                ...
                dialogs.extend(zip(dialog[:-1], dialog[1:]))
            # move question[-1]--<END> to answer[0], convert (q1<END>, a1) to (q1, <END>a1<END>)
            dialogs = [[q[:-1], [q[-1]] + a] for q, a in dialogs]
